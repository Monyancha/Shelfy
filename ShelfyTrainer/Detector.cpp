...
int Detector::loadImage(string filename, std::string filepath) {
...
//    Mat grayImg = getGrayImage(img);
    ...
    for( size_t i = 0; i < circles.size(); i++ )
    {
        ...
        if (createDatasetMode){
            if (supervisedDatasetCreation(filename, int(i), ROI) == 1){ //quit the app
                return 1;
            }
        } else {
...
int Detector::supervisedDatasetCreation(string filename, int featureCount, Mat img) {
    int keyCode = waitKey();
    ...
    writeImage("../data/" + directory + "/" + filename + "_" + to_string(featureCount) + ".jpg", img);
    return 0;
}
